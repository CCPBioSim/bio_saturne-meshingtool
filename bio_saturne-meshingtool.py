...
class UnsupportedError(Exception):
    def __init__(self, unsupported=None, supported=None):
        ...
        self.message = '\n----------------Unsupported Error----------------\n'\
        +"We do not current support the " + unsupported + " please try again using one of: "+ prt_supported
        super().__init__(self.message)
...
def launcher(cmd, ig_error=False):
    ...
    lstdout = []
    lstderr = []
    if isinstance(cmd[0], list) == False:
        cmds = [cmd]
    else:
        cmds = cmd
    end = len(cmds)
    while ind < end:
        ...
        elif ig_error == True and len(stdout.decode('utf-8')) == 0:
...
def check_software_install(software_name, version):
    ...
    if which_software!= "" and which_software!= None:
        path = which_software
    else:
    ...
    if current_version == None or not(version in current_version):
        raise SoftwareNotFound(software_name, version)
    return path
...
def process_gmsh_error(err, out, input_name, log_path):
    ...
    if err_num > 0:
        ...
        elif 'overlapping facets' in err and 'No elements in volume' in err:
...
def gmsh_from_stl(soft_dict, mesh_config_dict, input_filepath, input_name, log_foldr, mesh_filename, mesh_name):
    meshing_options = mesh_config_dict.keys()
    opts_lst = []
    for opt in meshing_options:
        if not opt in ['software', 'format', 'name']:
            opts_lst.append('-'+opt)
            opts_lst.append(str(config_dict[opt]))
    ...
    mesh_cmd = [soft_dict['gmsh'][1]]+ opts_lst+['-3','-o', mesh_filename,'-format', mesh_config_dict['format'],geofile, '-log', log_file]
    mesh_out, mesh_err = launcher(mesh_cmd, True)
    if mesh_err != None:
        process_gmsh_error(mesh_err, mesh_out, input_name, log_file)
    print("Volumetric mesh (", mesh_filename,") generated with", find_nodes_elements(mesh_out, log_file))
...
def change_user_script(study_name, case_name):
    ...
    checksed_cmd = ['sed', '-n', r"%sp" % str(line_num), study_name+'/'+ case_name+'/DATA/cs_user_scripts.py']
    checksed_out, checksed_err = launcher(checksed_cmd)
    #Checks file is successfully edited
    if not 'domain.mesh_input = "../MESH/mesh_input.csm"' in  checksed_out:
        error_message = "Error setting domain.mesh_input = mesh_input.csm in "+study_name+"/"+ case_name+"/DATA/cs_user_scripts.py\nPlease check the back-up file (/"+study_name+"/"+case_name+"/DATA/cs_user_scripts.py.bak at line "+ line_num
        raise NotFoundinFile('domain.mesh_input = "../MESH/mesh_input.csm"', 'cs_user_scripts.py', error_message)

#Runs post-volume using CodeSaturne's preprocessor which outputs information on a given volumetric mesh   
def cs_generate_volume(cs_prepro_path, mesh_filename, log_foldr):
    mesh_name, mesh_exten = get_name_and_exten(mesh_filename)
    cs_cmd = [cs_prepro_path,'--log', log_foldr+'/'+mesh_name+'_cspreprocessor.log','--post-volume',mesh_filename] 
...
def cs_prepare_files(study_name, case_name, cs_path):
    ...
    #Change script file to point at csm mesh assuming the first occurance of 'domain.mesh_input = None' is the line to change
...
def cs_run_quality(cs_path, study_name, case_name, wd_name):
    ...
    cp_mesh_cmd = ['cp', study_name+'/'+ case_name+'/SRC/REFERENCE/cs_user_mesh.c', study_name+'/'+ case_name+'/SRC']
    #Run the data preparation stage 
    run_init_cmd = [cs_path, 'run', '--case', study_name+'/'+ case_name, '--id', wd_name, '--initialize']
    #Run the solver
    run_solv_cmd = ['cs_solver', '-wdir', study_name+'/'+ case_name+'/RESU/'+wd_name+'/', '--quality']
    ...
    if not 'run_solver.log' in solv_files:
        raise CodeSaturneError('running cs_solver --quality', 'Check for the generation of run_solver.log when running \
        cs_solver script in', study_name+'/'+ case_name+'/RESU/'+wd_name+'/')
...
def extract_configs(yaml_file, input_exten, soft_dict):
    ...
    'name':[['all'], 'mesh'], 'threshold':[['map', 'emd'], 'map'], 'dust_filter':[['map', 'emd'], 'map']\
    , 'probe_radius': [['pdb'],'chi'], 'grid_spacing':[['stl', 'pdb', 'map', 'emd'], 'chi']}
    loader = yaml.Loader
    meshing_soft = {}
    ...
    if set(['software','format']).issubset(user_configs) == False:
        raise InputError("configurations", "\nPlease specify the meshing software' and 'format' in the configuration file")
    ...
    for uc in user_configs:
        if accepted_configs_dict[uc][0] != ['all'] and input_exten not in accepted_configs_dict[uc][0]:
            raise InputError('configurations', "\nThe argument '"+uc+"' given in the"+
            " configuration file is not permitted for inputs of the format "+ input_exten)
...
def generate_histograms(quality_file, hist_title_lines, hist_start_lines, hist_end_lines, mesh_name):
    hist_count = 0
...
def preprocess_hist_data(quality_file, mesh_name):
    ...
    if len(hist_data_start) != len(hist_data_end):
        raise NotFoundinFile('an equal number of start and end data points for histograms', quality_file)
    elif len(min_vals) != len(max_vals):
        raise NotFoundinFile('an equal number of maximum and minimum values for histograms', quality_file)
    elif len(hist_titles)!= len(min_vals):
        raise NotFoundinFile('maximum and minimum values for all histograms', quality_file)
    elif len(hist_data_end) != len(hist_titles):
        hist_titles, min_vals, max_vals = remove_hist_without_data(hist_titles_out, hist_titles, hstart_end_out[0], min_vals, max_vals)
    hist_titles, hist_data_start, hist_data_end = remove_hist_min_max(hist_titles, min_vals, max_vals, hist_data_start, hist_data_end)
    return hist_titles, hist_data_start, hist_data_end
...
def process_cs_quality(quality_file, save_hist, mesh_name):
    ...
    if save_hist == True:
        ...
        print("Histograms successfully generated and stored as pdfs in /"+mesh_name+'_quality/'+mesh_name + '_histograms') 
...
def mesh_filename_preexist(mesh_name, mesh_exten):
    ...
    if mesh_filename in lsout:
        ...
        print("WARNING: File of the name", mesh_filename,"already exists in the current directory")
        while not(cont == 'y' or cont =='n'):
            cont = input("Enter 'Y' to continue and overwrite the file or 'N' to provide a new file name: ")
            cont = cont.lower()
...
#Replaces any spaces in the given filename with underscores and checks it doesn't include an extension
def format_mesh_filename(mesh_name, mesh_exten):
    mesh_filename = mesh_name.replace(' ', '_')
    if not '.' in mesh_name:
        mesh_filename = mesh_name+'.'+mesh_exten
    else:
        raise InputError('mesh name', "check the 'name' field in the configuration file (this name shouldn't include an extension)")
...
def check_input_args(input_format, inp, supported_input, soft_dict):
    ...
    if input_format == 'emd':
        ...
        if format_chk == False:
            raise InputError('emd entry', r"\nPlease enter the input in the format: emd_{entry number} e.g. emd_3066")
    else:
        ...
        if input_exten != input_format:
            raise InputError('input file', '\nPlease ensure the input file is saved with the appropriate \
            extension specified in --format')
...
def download_emd(emd):
    ...
    emd_cmd = ['rsync', '-rlpt', '-v', '-z', '--delete', 'rsync.ebi.ac.uk::pub/databases/emdb/structures/EMD-' \
    ...
    for config in map_configs:
        ...
        if config == 'dust_filter':
            if map_config_dict[config].lower() == 'true':
                ...
                raise InputError('configurations', "\nInvalid value for argument 'dust_filter' in the" +
                " configuration file. This must be True or False")
        #Check threshold argument given is a number and add the command to perform this
        if config == 'threshold':
            if isnumber(map_config_dict[config]) == True:
                config_cmd = config_cmd + ['-t', str(map_config_dict[config])]
            else:
                raise InputError('configurations', "\nInvalid value for argument 'threshold' in the"+
                " configuration file. This must be an integer or float")
    ccpem_cmd = ['ccpem-python', ccpem_path[0:-10]+'lib/py2/ccpem/src/ccpem_core/map_tools/TEMPy/map_preprocess.pyc','-m', map_filepath] \
...
def main():
    ...
    parser.add_argument("-c","--configs", required = False, help="file name (and path) to configuration yaml file")
    #Histograms and visualisation are optional flags
    parser.add_argument("-hg", "--histograms", required = False, help="flag to generate histograms to assess mesh quality", action = "store_true")
    parser.add_argument("-v","--visualise", required =False, help="Generates a visualisation of the surface in Paraview", action = 'store_true')
    ...
    if args.configs != None:
        ...
        meshing_soft, mesh_config_dict, map_config_dict, chi_config_dict = extract_configs(args.configs, input_exten, soft_dict)
        ...
        if 'name' in mesh_config_dict:
            mesh_filepath = check_mesh_filename(mesh_config_dict['name'], mesh_config_dict['format'], input_name)
        else:
    ...
    if input_exten == "emd" or input_exten == "map":
        ...
        if map_config_dict != {}:
            map_filepath = ccpem_cleaning(soft_dict['ccpem'][1], map_filepath, map_name, map_config_dict)
        input_name = to_stl(soft_dict['ucsf-chimerax'][1], map_filepath, map_name, map_exten, chi_config_dict)
        input_exten = 'stl'
        ...
        input_name = to_stl(soft_dict['ucsf-chimerax'][1], input_filepath, pdb_name, 'pdb', chi_config_dict)
        input_exten = 'stl'
        input_filepath = input_name + '.' + input_exten
    ...
    if input_exten == "stl":
        ...
        if mesh_config_dict['software'] == 'gmsh':
            print("\n----------------GMSH----------------")
            gmsh_from_stl(soft_dict, mesh_config_dict, input_filepath, input_name, log_foldr, mesh_filepath, mesh_name)
    ...
    print("\n----------------CODESATURNE----------------")
    quality_file = cs_prepro_quality(soft_dict['cs_preprocess'][1], soft_dict['code_saturne'][1], mesh_filepath, log_foldr)
    ...
    print("\nFurther files generated by intercalated software is stored in .tmp")
    exit()
